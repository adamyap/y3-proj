...
def run_image_processing():
    ...
    global ball_last_moving_time  # Ensure it's accessible and reset if needed
    ...
    while True:
        ...
        if ball_is_stuck(velocity):  # You'll need to define the logic for ball_is_stuck
            induce_vibration()
...
def induce_vibration():
    vibration_sequence = [('A', 100), ('B', 100), ('A', -100), ('B', -100)] * 5
    for motor, position in vibration_sequence:
        send_position(motor, position)
        time.sleep(0.05)  # Adjust timing based on your needs
    
def ball_is_stuck(velocity):
...
def PIDcontrol(Kp,Ki,Kd,distance,integral,velocity):
    e = (Kp.get())*distance + (Ki.get())*integral + float(Kd.get())*(velocity)
    if e < 0:
        e -= 50
    elif e > 0:
        e += 50
